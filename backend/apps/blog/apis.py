...
async def read_article2(id: int):
    article = await site.db.async_get(Article, id, options=[selectinload(Article.user)])
    print("user", article.user)
    return article
...
async def read_article3(id: int):
    article = await site.db.session.get(Article, id, options=[selectinload(Article.user)])
    print("user", article.user)
    return article
...
@router.delete("/delete3/{id}", response_model=int, summary="删除文章")
@auth.requires(roles="admin")  # 要求必须登录,并且是管理员角色
async def delete_article3(request: Request, id: int):
    stmt = delete(Article).where(Article.id == id)
    result = await site.db.session.execute(stmt)
    return result.rowcount
